...
- Added `LOG_PROBABILITY`, the log-sum-exp semiring over finite IEEE-double log
  ...
  positive weights admitted, overflow refused, every accumulation law declared
  approximate, and no star. `tiergraph semirings` lists it as
...
- Added `PathPlan`, which compiles a `FoldDeclaration` over one `OR` relation on
  ...
  outside pass and returns `PathMarginals`, whose `posteriors` reads probabilities
  through the algebra's `normalize` and reports zero mass rather than fabricating
  a distribution, as `PathPosteriors` with the readout named. Under
